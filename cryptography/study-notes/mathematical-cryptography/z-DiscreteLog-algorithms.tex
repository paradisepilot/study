...
\textbf{Theory:}
\begin{itemize}
\item	We are seeking $x$ such that $g^{x} = h$. Note that $x$ is well-defined modulo $N$.
\item	If we know the prime factorization of $N = p_{1}^{\alpha_{1}}\,p_{2}^{\alpha_{2}}\,p_{3}^{\alpha_{3}}\,\cdots\,p_{t}^{\alpha_{t}}$,
		...
		\begin{equation*}
		y_{i} \in \{\,1,2,\ldots,p_{i}^{\alpha_{i}}-1\,\}
		\end{equation*}
		to the following associated problems:
		\begin{equation*}
		g_{i}^{y_{i}} \; = \; h_{i},
		...
		\quad\textnormal{for}\;\; t = 1,2,\ldots,t\,.
		\end{equation*} 
\end{itemize}
